      ...
      <div>
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2.5 border-1-3 transition-all duration-200 ${isActive ? "bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400 font-medium" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200 border-transparent"}`
            }
          >
